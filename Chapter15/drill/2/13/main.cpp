...
template <int... Numbers>
std::vector<int> make_vector(Sequence<Numbers...>)
{
	return std::vector<int>{Numbers...};
}
...
int main()
{
	...
	auto v1 = make_vector(MakeSequence<1, 10>{});
	auto v2 = std::vector<int>(10);
	for (decltype(v1)::size_type i = 0; i < v1.size(); ++i)
	{
		v2[i] = v1[i];
	}
}
