...
(Vector_ref<Regular_hexagon>& vec, Point& center, int distance, int width, int height) {
    ...
    for (int i = 0; i < height; ++i) {
        if (i) {    // Determine next row's first hexagon's position
            TileDirection connector = (i % 2) ? TileDirection::BOTTOM_LEFT : TileDirection::BOTTOM_RIGHT;
...
int main(int /*argc*/, char * /*argv*/[])
{
    ...
    Point center{2 * distance, distance};

    // Generate grid
    place_hex_grid(vec, center, distance, 6, 6);

    {   // Attach and stylize hexagons
    int i = 0;
        for (Regular_hexagon* p : vec) {
            Color fill = (i % 2) ? Color::dark_cyan : Color::yellow;
