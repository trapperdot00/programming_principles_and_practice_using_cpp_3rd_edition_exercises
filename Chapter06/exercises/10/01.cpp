...
struct Token {
	Token(char c) : kind(c) {}
	Token(char c, int d) : kind(c), val(d) {}
...
struct Variable {
	Variable(std::string s, int d, bool c) : name(s), value(d), constant(c) {}
	std::string name;
	int value;
	bool constant;
};
...
public:
	...
	int declare(const std::string&, int, bool);
private:
	std::vector<Variable> var_table;
} symbols;	// Create a global object

// Get variable's value by its name
int Symbol_table::get(const std::string& name) const {
...
void Symbol_table::set(const std::string& name, int value) {
...
int Symbol_table::declare(const std::string& name, int value, bool constant) {
	if (is_declared(name))
...
Token Token_stream::get() {
	...
	case '5': case '6': case '7': case '8': case '9':	// Integer
	{
		...
		return Token{numeric, static_cast<int>(d)};
	}
	default:
	{
...
int primary() {
	...
	case tpow:
	{
		...
		if (p > 0) {
			for (int i = 1; i < p; ++i) {
				if (old > 0 && d > std::numeric_limits<int>::max() / old)
					throw std::runtime_error("pow: overflow");
				if (old < 0) {
					if (i % 2 && d <= std::numeric_limits<int>::max() / old)
						throw std::runtime_error("pow: overflow");
					if (!(i % 2) && d >= std::numeric_limits<int>::min() / old)
						throw std::runtime_error("pow: underflow");
				}
			...
			return 0;	// Negative power doesn't work for integers
		}
	}
	default:
