...
double define_name(const std::string&, double, bool);
...
double declaration(Token t) {
	...
	define_name(t1.name, value, t.kind==tconstdecl);
	return value;
}
...
double define_name(const std::string& name, double value, bool constant) {
	if (is_declared(name))
