...
public:
    ...
    Binary_tree(Point c, int r, int levels, Line_style l = Line_style::solid,
        ...
        : ra{r}, y_step{4 * ra}, x_step{y_step}, ls{l}, col{co}, dir{d}
    {
    ...
    void add_text(const std::string& node_id, const std::string& text) {
        if (node_id.empty())
            throw std::runtime_error{"direction string cannot be empty"};
        if (node_id[0] != 'l' && node_id[0] != 'r')
            throw std::runtime_error{
                "invalid characters in direction string "
                "(valid characters: l,r)"
            };
        size_t index = 0;
        for (size_t row = 1, column = 0; row < node_id.size(); ++row, column *= 2) {
            size_t nodes_in_row = std::pow(2, row);
            if (node_id[row] == 'r')
                ++column;
            else if (node_id[row] != 'l')
                throw std::runtime_error{
                    "invalid characters in direction string "
                    "(valid characters: l,r)"
                };
            index = nodes_in_row - 1 + column;
        }
        if (n.size() <= index)
            throw std::runtime_error{
                "direction string out of range"
            };
        Point p = n[index];
        t.push_back(std::make_unique<Text>(Point{p.x - ra, p.y + 2 * ra}, text));
    }
protected:
    ...
    virtual void draw_text(Painter& painter) const {
        for (Text* p : t)
            p->draw(painter);
    }
...
void generate_combinations(const std::string& chars,
                           size_t depth,
    ...
    if (current.size() == depth)
        return;
    for (char c : chars)
        generate_combinations(chars, depth, current + c, vec);
}

std::vector<std::string> combinations(const std::string& chars, size_t depth) {
    std::vector<std::string> v;
    generate_combinations(chars, depth, "", v);
    return v;
}
...
int main(int /*argc*/, char * /*argv*/[])
{
    ...
    Binary_tree bt(Point{400, 100}, 26, 4, Line_style{Line_style::solid, 2},
                   Color::black, Binary_tree::Arrow_direction::down
    );

    bt.add_text("l", "l");
    std::vector<std::string> directions = combinations("lr", 3);
