...
struct Styles_window : Color_window {
    Styles_window(Application& app, Point p, int w, int h, const std::string& title)
    :   Color_window{app, p, w, h, title},
        styles_button{Point{w - 80, h / 2}, 80, 20, "style menu", [this]{ show_menu(); }},
        styles_menu{Point{w - 80, h / 2 + 10}, 80, 5*20, Menu::vertical, "styles"}
    {
        attach(styles_button);
        styles_menu.attach(std::make_unique<Button>
            (Point{0, 0}, 0, 0, "solid", [this]{ change(Line_style::solid); hide_menu(); })
        );
        styles_menu.attach(std::make_unique<Button>
            (Point{0, 0}, 0, 0, "dash", [this]{ change(Line_style::dash); hide_menu(); })
        );
        styles_menu.attach(std::make_unique<Button>
            (Point{0, 0}, 0, 0, "dashdot", [this]{ change(Line_style::dashdot); hide_menu(); })
        );
        styles_menu.attach(std::make_unique<Button>
            (Point{0, 0}, 0, 0, "dashdotdot", [this]{ change(Line_style::dashdotdot); hide_menu(); })
        );
        styles_menu.attach(std::make_unique<Button>
            (Point{0, 0}, 0, 0, "dot", [this]{ change(Line_style::dot); hide_menu(); })
        );
        hide_menu();
        attach(styles_menu);
    }
private:
    void change(Line_style ls) {
        lines.set_style(ls);
    }
    void show_menu() {
        styles_button.hide();
...
int main() try {
    Application app;
    Styles_window win(app, {100, 100}, 600, 400, "lines");
    app.gui_main();
